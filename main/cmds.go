...
func enable(ctx *log.Context, h HandlerEnvironment, report *RunCommandInstanceView, extName string, seqNum int) (string, string, error, int) {
	...
	if err != nil {
		return "", "",
			...
			ExitCode_DownloadArtifactFailed
	}
...
func downloadScript(ctx *log.Context, dir string, cfg *handlerSettings) (string, error) {
	...
	if scriptURI != "" {
		...
		file, err := downloadAndProcessScript(ctx, scriptURI, dir, cfg)
		if err != nil {
...
func downloadArtifacts(ctx *log.Context, dir string, cfg *handlerSettings) error {
	...
	for i := 0; i < len(artifacts); i++ {
		...
		filePath, err := downloadAndProcessArtifact(ctx, dir, &artifacts[i])
		if err != nil {
